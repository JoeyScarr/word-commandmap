      ...
      private void SetUpHooks()
      {
		...
		internal void UpdateRegions(Graphics g)
		{
			...
			if (RightToLeft == RightToLeft.No)
			{
				...
				if (CaptionButtonsVisible)
				{
					int width = (int)Math.Ceiling(g.MeasureString(CloseButton.Text, new Font(RibbonCaptionButton.WindowsIconsFont, Font.Size)).Width) + 3;
					Size cbs = new Size(width, 20);
					int cbg = 2;
            ...
            else
            {
                ...
                foreach (RibbonTab tab in Tabs)
                {
                    if (tab.TabBounds.Contains(e.X, e.Y))
                    {
                        SetSelectedTab(tab);
                        someTabHitted = true;
                        tab.OnMouseMove(e); // @ adriancs: why did you remove the MouseMove event on the tab? ToolTip will not get active!
                    }
                }
            }
