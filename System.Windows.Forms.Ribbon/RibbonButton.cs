...
namespace System.Windows.Forms {
	...
	public class RibbonButton : RibbonItem, IContainsRibbonComponents {
		...
		public static Size MeasureStringLargeSize(Graphics g, string text, Font font) {
			...
			Size oneLineSize = g.MeasureString(text, font).ToSize();
			if (text.Contains(" ")) {
				// The command name contains more than one word, so try to fit on two lines.
				// There are some cases where this won't work, but we don't have any of those.
				return g.MeasureString(text, font, oneLineSize.Width - 1).ToSize();
			} else {
				return oneLineSize;
			}
		}
